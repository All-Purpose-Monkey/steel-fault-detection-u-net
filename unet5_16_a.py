...
def build_unet(input_shape=(256, 1600, 1), num_classes=4):
    ...
    c2 = encoder_block(c1, 32)
    c3 = encoder_block(c2, 64)
    c4 = encoder_block(c3, 128)
    ...
    c7 = decoder_block(c6, c4, 128)
    c8 = decoder_block(c7, c3, 64)
    c9 = decoder_block(c8, c2, 32)
...
def generalized_dice_loss(num_classes, smooth=1, mode="rt",power=0.5):
    def loss(y_true, y_pred):
        ...
        gdl = (2.0 * intersection + smooth) / (denom + smooth)

        return 1.0 - tf.reduce_sum(weights * gdl) / tf.reduce_sum(weights)
...
if __name__ == "__main__":
    ...
    # SAVE MODEL
    # -------------------------
    model.save("steel_unet5_16a_gdl_wbce_model.keras")
    model.save_weights("steel_unet5_16a_gdl_wbce_weights.weights.h5")

    print("\n✅ Model saved successfully!")

    # Save history to CSV in root directory
    history_df = pd.DataFrame(history.history)
    history_df.to_csv("unet5_16a_gdl_wbce_training_history.csv", index=False)
